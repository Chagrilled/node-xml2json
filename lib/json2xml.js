...
ToXml.prototype.parse = function(obj, depthIndex = 0) {
    if (!obj) return
    var self = this;
        ...
        if (Array.isArray(obj[key])) {
            ...
            for (var j = 0; j < l; j++) {
                ...
                if (typeof(elem) == 'object') {
                    self.openTag(key, depthIndex);
                    self.parse(elem, (new Number(depthIndex) + 1));
                    self.closeTag(key, depthIndex);
                }
            }
        } else if (typeof(obj[key]) == 'object' && !(self.options.ignoreNull && obj[key] === null)) {
            self.openTag(key, depthIndex);
            self.parse(obj[key], (new Number(depthIndex) + 1));
            self.closeTag(key, depthIndex);
        }
    }
};

ToXml.prototype.openTag = function(key, identIdx = 0) {
  this.completeTag();
  if (this.xml && this.xml.length > 0) {
    this.xml += '\r\n';
  }
  for (let i = 0; i < identIdx; i++) {
    this.xml += '\t';
  }
  this.currTAG = key;
  this.xml += '<' + key;
...
ToXml.prototype.closeTag = function(key, identIdx = 0) {
  this.completeTag();
  if (this.currTAG !== key) {
    this.xml += '\r\n';
    for (let i = 0; i < identIdx; i++) {
